...
try {
    $stmt = $db->prepare("SELECT description FROM services WHERE description IS NOT NULL LIMIT 1");
...
<head>
    ...
    <title>Instagram Hizmetleri - Bayigram</title>
...
<div class="bayigram-services-grid">
    <?php
    try {
        $stmt = $db->prepare("SELECT id, name FROM services2");
        $stmt->execute();
    ...
    foreach ($services as $service) {
        echo '<div class="bayigram-service-card">
                ...
                <a href="siparis.php?id=' . $service['id'] . '" class="bayigram-view-packages-btn">Satın Al</a>
              </div>';
    }
    ?>
</div>
